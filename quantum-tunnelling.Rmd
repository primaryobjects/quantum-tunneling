...
The analaysis includes a select data-set of 9 examples by year and semiconductor barrier size. Additionally, the data-set includes projected estimates for semiconductor barrier sizes for the years 2019 through 2025 and beyond.

## Background

Quantum tunneling is the process by which a tiny particle, such as an electron, can pass through a solid barrier given certain conditions.

The quantum tunneling effect manifests itself within a quantum system at the microscopic scale. This is generally considered within the range of nanometers. While the macroscopic world that we can see and feel around us behaves in traditionally expected ways according to the laws of classical physics, the microscopic world behaves much differently. Specifically, quantum physics demonstrates effects including entangelement, teleportation, and tunneling by very small particles such as electrons and photons.

Since an electron travelling through and beyond a semiconductor barrier would result in data loss of the bit, this process will become increasingly important to computing as technology continues its advancement to minituraized components. Beyond a certain level of miniaturization, these components will begin to exhibit quantum effects, resulting in the probability of an electron tunneling through a barrier within a transistor becoming increasingly more probable.

As a result of tunneling, an electron passing through a transistor could be lost, resulting in loss of data. Given the steady progress of miniturization of transistors and CPUs, it is possible to begin seeing the effects of quantum tunneling on traditional computers within the next 10 years.

## Quantum Tunneling Requirements

In order for quantum tunneling to occur, the following requirements must exist.
...
The key question in quantum effects of classical computer transistors is how soon could quantum tunneling become a large enough problem to prevent computers from operating efficiently. According to the above estimated probability data, based upon the current semidonductor manufactoring process and future expections, it appears around the year `r max(prob$Year)` we may begin to see an unreasonable amount of data loss from classical computers built with transistors. This data loss could be expected to be in the range of `r round(prob[prob$Year==2025,]$Probability * 100, 2)`%.

Note, the above data based upon estimates and should not be considered as a given prediction of the state of quantum tunneling on computing technology.
